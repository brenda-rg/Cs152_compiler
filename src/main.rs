...
fn main() {
    ...
    let tokens = match lex(&code){
      ...
      println!("Finished file {} lexation",filename);
      for i in &tokens{
        println!("{:?}",i); // printing out the tokens
      }
    }
...
fn lex_equality(code: &str) -> (bool, Token, &str) {
  ...
  for letter in code.chars() {
    ...
    StateMachine::Sign => {
      if letter == '=' {
        ...
        let token = &code[..index];
        return (true, create_operator(token), &code[index..]);
      }
    }
    StateMachine::Sign2 => {
      let token = &code[..index];
  ...
  if success == true {
    return (true, create_operator(code), "");
  } else {
    return (false, Token::NotToken, "");
  }
}
...
fn create_operator(code: &str) -> Token{
  match code {
    "<" => Token::Less,
    ">" => Token::Greater,
    "<=" => Token::LessEqual,
